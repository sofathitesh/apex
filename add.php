...
function checkStateChange(){$("#sub_section select").on("focus",function(){$(this).change(function(){chkSubject($(this).data("i"),$("#"+$(this).attr("id")).val())})})}function getPackSection(a){$.ajax({type:"POST",url:'<?=base_url("index.php/student/getPackSection")?>',data:"id="+a,dataType:"html",success:function(a){sectionID=a,$("#sectionID").html('<input type="hidden" name="sectionID" id="sectionID" value="'+a+'">'),$(".sel_sub").each(function(){$(this).val(a).trigger("change")})},async:!1})}function chkSubject(a,b){var c=$("#classesID1").val();0!==b&&0!==c&&$.ajax({type:"POST",url:'<?=base_url("index.php/student/checkSubjectAvailable")?>',data:"subID="+a+"&classesID="+c+"&secID="+b,dataType:"html",success:function(b){"err"===b?($("#section"+a).parent().parent().addClass("has-error"),$("#error"+a).html("Subject full")):($("#section"+a).parent().parent().removeClass("has-error"),$("#error"+a).html(""))},async:!1})}$("#dob").datepicker({startView:2,dateFormat:"dd-mm-yy"});var sections="",sectionID;$("#classesID1").change(function(a){var b=$(this).val();"0"===b?$("#classesID").val(0):($.ajax({type:"POST",url:'<?=base_url("index.php/student/sectioncall")?>',data:"id="+b,dataType:"html",success:function(a){sections=a}}),$.ajax({type:"POST",url:'<?=base_url("index.php/student/callPackage")?>',data:"id="+b,dataType:"html",success:function(a){$("#packages").html(a),$("#sub_section").html("")}}),$("#add_p").html(""),$.ajax({type:"POST",url:'<?=base_url("student/callAdditionalPackage")?>',data:"id="+b,dataType:"html",success:function(a){var b='<select id="additional_pack" class="form-control" name="additional_pack[]" multiple="multiple">'+a+"</select>";$("#add_p").html(b),setTimeout(function(){$("#additional_pack").multiselect({includeSelectAllOption:!0})},300)}}))}),$("#packages").change(function(){var a=$(this).val();"0"===a?$("#sub_section").html(""):$.ajax({type:"POST",url:'<?=base_url("index.php/student/packagesubjectcall")?>',data:"packageID="+a,dataType:"html",success:function(b){$("#sub_section").html("");var c=JSON.parse(b),d=c.subs,e=c.overs;for(var f in d)e.includes(f)?$("#sub_section").append('<div class="form-group has-error"> <label for="sub'+f+'" class="col-sm-4 control-label"> '+d[f]+' </label> <input type="hidden" name="subject[]" value="'+f+'"> <div class="col-sm-6"> <select name="section[]" id="section'+f+'" class="form-control sel_sub" data-i="'+f+'">'+sections+'</select> </div><span class="control-label" id="error'+f+'"> Subject full</span> </div>'):$("#sub_section").append('<div class="form-group"> <label for="sub'+f+'" class="col-sm-4 control-label"> '+d[f]+' </label> <input type="hidden" name="subject[]" value="'+f+'"> <div class="col-sm-6"> <select name="section[]" id="section'+f+'" class="form-control sel_sub" data-i="'+f+'">'+sections+'</select> </div><span class="control-label" id="error'+f+'"> </span> </div>');checkStateChange(),getPackSection(a)}})});
</script>
<!--
